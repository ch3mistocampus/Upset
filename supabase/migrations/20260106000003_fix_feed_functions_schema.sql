-- Fix feed functions to match actual database schema
-- Activities table has: type (not activity_type), no title/description/is_public
-- Picks table has: status (not is_correct)
...
-- Recreate get_discover_feed with correct schema
...
-- Recreate get_following_feed with correct schema
...
-- Recreate get_trending_users with correct schema (using status instead of is_correct)
